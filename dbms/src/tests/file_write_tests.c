...
static Status File_WriteTyped_AddSectionAndWrite(Json *json)
{
    ...
    ENTITY_INIT(entity, json, 0, 0, 0, 0);
    ...
    sect_ext_read(file->f_extent->head, sect_head_get_sectoff((Sect_head *)file->f_extent->head, wrt_addr), o_entity, o_json);
    ...
    assert(o_entity->init_rec_sz == entity->key_size + entity->val_size);
    ...
    {
        memcmp(json_val_ptr(o_json), json_val_ptr(json), json_val_size(o_json));
    }
...
// static Status File_WriteTwoStrings_AddSectionAndWriteTwoObjectsInOneSect()
// {
//     FILE *filp = fopen(test_file_name, "w+");
...
// }
// */
// static Status File_WriteObject_AddSectionAndWriteComponents()
// {
//     FILE *filp = fopen(test_file_name, "w+");
...
// static Status File_WriteObjectInFileWithExistExtentsion_WriteComponents()
// {
//     FILE *filp = fopen(test_file_name, "w+");
...
// */
// static Status File_WriteObjectInFileWithExistFilledExtentsion_AddSectionWriteComponents()
// {
//     FILE *filp = fopen(test_file_name, "w+");
...
// 5) amount
// */
// static Status File_WriteThreeLevelsObject_AddSectionWriteComponents()
// {
//     FILE *filp = fopen(test_file_name, "w+");
...
void test_file_write()
{
    ...
    JSON_VALUE_INIT(TYPE_STRING, str_json, "value", str);
    assert(File_WriteTyped_AddSectionAndWrite(str_json) == OK);
    JSON_VALUE_INIT(TYPE_INT32, int_json, "value", 5);
    assert(File_WriteTyped_AddSectionAndWrite(int_json) == OK);
    JSON_VALUE_INIT(TYPE_BOOL, bool_json, "value", true);
    assert(File_WriteTyped_AddSectionAndWrite(bool_json) == OK);
    JSON_VALUE_INIT(TYPE_FLOAT, float_json, "value", 5.5);
    assert(File_WriteTyped_AddSectionAndWrite(float_json) == OK);
