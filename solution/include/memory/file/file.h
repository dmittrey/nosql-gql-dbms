...
{
    file_head_t header;
    struct sect_ext_t *f_extent;
    struct sect_type_t *f_types;
    FILE *filp;
} file_t;
...
status_t file_find_type(file_t *const file, const string_t *const name, type_t *const o_type, struct sect_type_t **o_types);
...
status_t file_find(file_t *const file, struct sect_ext_t *section, const query_t *const query, list_json_t *const o_obj_col);

status_t file_add_sect_ext(file_t *const file, struct sect_ext_t *r_extents);
status_t file_add_sect_types(file_t *const file, struct sect_type_t *r_types);